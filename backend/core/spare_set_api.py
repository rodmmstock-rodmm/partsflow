...
def stock_map_for(part_ids):
    ...
    return {row["id"]: float(row["stock_qty"] or 0) for row in rows}
        ...
        "items": [
            {
                ...
                "stock_qty": stock_map.get(item.part_id, 0),
...
def parse_items(items_data):
    ...
    for index, raw in enumerate(items_data, start=1):
        ...
        if not part:
            raise ValueError(
                f"รายการที่ {index}: ไม่พบอะไหล่"
            )
        quantity = as_positive_decimal(
            raw.get("quantity", 1), f"จำนวนรายการที่ {index}"
        )
        parsed.append(
            {
                "part": part,
...
def spare_sets(request):
    permission = (
        "can_view_dashboard" if request.method == "GET" else "can_edit_parts"
    )
    ...
    if request.method == "GET":
        ...
        qs = MachineSpareSet.objects.select_related(
            "machine", "created_by_employee"
        ...
        if q:
            qs = qs.filter(
                ...
                | Q(items__part__sku__icontains=q)
                | Q(items__part__name__icontains=q)
            ).distinct()
    ...
    try:
        ...
        with transaction.atomic():
            spare_set = MachineSpareSet.objects.create(
                machine=machine,
                name=name,
            ...
            result = spare_set_json(
                MachineSpareSet.objects.select_related(
                    "machine", "created_by_employee"
                ).get(pk=spare_set.pk)
            )
        ...
        detail = (
            "เครื่องจักรนี้มีชื่อ Set นี้อยู่แล้ว"
...
def spare_set_detail(request, pk):
    ...
    spare_set = MachineSpareSet.objects.select_related(
        "machine", "created_by_employee"
    ).filter(pk=pk).first()
    if not spare_set:
        ...
        with transaction.atomic():
            if "machine_id" in request.data:
                machine = Machine.objects.filter(
                    pk=request.data.get("machine_id"), active=True
                ).first()
                if not machine:
