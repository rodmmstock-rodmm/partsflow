...
function DesktopShell() {
  ...
  return <div className="app-shell">
    ...
    <aside className={sidebarOpen ? "sidebar open" : "sidebar"}>
      ...
      <nav className="nav-list">{visible.map(([to,label,icon])=><NavLink key={to} to={to} end={to==="/"} onClick={()=>setSidebarOpen(false)} className={({isActive})=>isActive?"nav-link active":"nav-link"}><span className="nav-icon">{icon}</span><span>{label}</span></NavLink>)}</nav>
