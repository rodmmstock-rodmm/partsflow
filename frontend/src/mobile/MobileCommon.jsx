...
export function MobileStockModal({ mode, part, employee, onClose, onSaved }) {
  ...
  async function submit(e){
    ...
    if(issue&&!requesterId){setError("กรุณาเลือกผู้เบิก");return;}
  ...
  return <Modal title={issue?"เบิกอะไหล่":"รับเข้าอะไหล่"} onClose={onClose}>
    <form className="m-stock-form" onSubmit={submit}>
      ...
      {issue&&<>
        <label className="field"><span>ผู้เบิก *</span><select value={requesterId} onChange={e=>setRequesterId(e.target.value)} disabled={loading}><option value="">เลือกผู้เบิก</option>{(options.employees||[]).filter(x=>x.active!==false).map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label>
