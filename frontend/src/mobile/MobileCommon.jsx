...
export function MobileStockModal({ mode, part, employee, onClose, onSaved }) {
  ...
  return <Modal title={issue?"เบิกอะไหล่":"รับเข้าอะไหล่"} onClose={onClose}>
    <form className="m-stock-form" onSubmit={submit}>
      ...
      {issue&&<>
        <label className="field"><span>ผู้เบิก *</span><SearchableSelect required value={requesterId} options={(options.employees||[]).filter(x=>x.active!==false)} onChange={setRequesterId} disabled={loading} getLabel={x=>`${x.employee_code?`${x.employee_code} · `:""}${x.name}`} getSearchText={x=>`${x.employee_code||""} ${x.name||""} ${x.department||""}`} placeholder="พิมพ์ชื่อหรือรหัสพนักงาน"/></label>
        <label className="field"><span>เครื่องจักร *</span><SearchableSelect required value={machineId} options={(options.machines||[]).filter(x=>x.active!==false)} onChange={setMachineId} disabled={loading} getLabel={x=>`${x.code} · ${x.name}`} getSearchText={x=>`${x.code||""} ${x.name||""} ${x.location||""}`} placeholder="พิมพ์ชื่อหรือรหัส Machine"/></label>
      </>}
