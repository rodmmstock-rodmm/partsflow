...
export default function MobileDashboard(){
  ...
  const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [stock,setStock]=useState(null); const [detail,setDetail]=useState(null); const timer=useRef(null);
  ...
  return <MobilePage title="Stock" subtitle="ค้นหา · เบิก · รับเข้า แบบมือถือ" actions={<button className="m-icon-action" onClick={()=>load(true)}>↻</button>}>
    ...
    <button className="m-dashboard-spare-set" onClick={()=>navigate("/spare-sets")}><span>⚙</span><div><b>ชุดอะไหล่เครื่องจักร</b><small>Machine Spare Sets</small></div><strong>›</strong></button>
    ...
    {loading?<MobileLoading/>:parts.length===0?<MobileEmpty/>:<div className="m-card-list">{parts.map(p=><article className="m-part-card" key={p.id}>
      ...
      <button className="m-detail-button" onClick={()=>setDetail(p)}>ดูรายละเอียดทั้งหมด</button>
