...
export default function RoleSettings(){
  ...
  {tab==="logs"&&<section className="panel"><div className="table-wrap"><table><thead><tr><th>Date Time</th><th>Employee</th><th>Action</th><th>Entity</th><th>Detail</th></tr></thead><tbody>{logs.map(l=><tr key={l.id}><td>{formatDMY(l.created_at, true)}</td><td>{l.employee?`${l.employee.employee_code} · ${l.employee.name}`:"-"}</td><td><b>{l.action}</b></td><td>{l.entity} {l.entity_id}</td><td><pre className="log-detail">{JSON.stringify(l.detail,null,2)}</pre></td></tr>)}</tbody></table></div></section>}
