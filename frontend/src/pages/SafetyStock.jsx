...
export default function SafetyStock(){
  ...
  return <>
    ...
    <section className="panel"><div className="toolbar"><input className="search-input" value={q} onChange={e=>setQ(e.target.value)} placeholder="ค้นหา Item ID / Part / Maker / Machine..."/><button className="btn ghost" onClick={()=>load(true)}>รีเฟรช</button></div>{loading?<div className="empty">กำลังโหลด...</div>:<div className="table-wrap"><table><thead><tr><th><input type="checkbox" checked={shown.length>0&&shown.every(x=>selected.has(x.id))} onChange={toggleAll}/></th><th>Item ID</th><th>Part Name</th><th>Part Detail</th><th>Maker</th><th>เครื่องจักรล่าสุดที่เบิก</th><th>จำนวนคงเหลือ</th><th>จำนวน Min</th><th>จำนวนที่ Order</th><th>Unit</th></tr></thead><tbody>{shown.map(x=><tr key={x.id}><td><input type="checkbox" checked={selected.has(x.id)} onChange={()=>toggle(x.id)}/></td><td><b>{x.sku}</b></td><td>{x.name}</td><td className="detail-cell">{x.description||"-"}</td><td>{x.maker_name||"-"}</td><td>{x.last_machine||"-"}</td><td className="text-danger"><b>{fmt(x.stock_qty)}</b></td><td>{fmt(x.min_stock)}</td><td><b>{fmt(x.order_qty)}</b></td><td>{x.unit_code}</td></tr>)}</tbody></table></div>}</section>
