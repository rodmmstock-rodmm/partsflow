  ...
  return <Modal title={edit ? "แก้ไขข้อมูลอะไหล่" : "เพิ่มรายการอะไหล่"} onClose={onClose} wide>
    <form onSubmit={save}>
      <div className="form-grid three">
        ...
        <label className="field span3"><span>Image URL / Google Drive URL</span><input value={form.image_path} onChange={e=>set("image_path",e.target.value)} placeholder="วางลิงก์รูป หรือ Google Drive share link..."/><small className="field-help">Google Drive ต้องแชร์ไฟล์เป็น “Anyone with the link” ระบบจะแปลง Share URL เป็นรูปให้อัตโนมัติ</small></label>
...
function StockModal({ mode, part, options, employee, onClose, onSaved }) {
  ...
  return <Modal title={issue?"เบิกอะไหล่":"รับอะไหล่เข้าสต็อก"} onClose={onClose}>
    <form onSubmit={save}>
      <div className="part-preview"><div className="part-thumb"><PartImage src={part.image_path} alt={part.name}/></div><div><strong>{part.sku}</strong><h3>{part.name}</h3><p>{part.description||"-"}</p><small>Stock {fmt(part.stock_qty)} {part.unit_code}</small></div></div>
...
export default function Dashboard(){
  const auth=useAuth();
  const tableScrollRef=useRef(null); const topScrollRef=useRef(null); const [tableScrollWidth,setTableScrollWidth]=useState(0);
  ...
  const syncTop=(e)=>{if(tableScrollRef.current)tableScrollRef.current.scrollLeft=e.currentTarget.scrollLeft};
  const syncTable=(e)=>{if(topScrollRef.current)topScrollRef.current.scrollLeft=e.currentTarget.scrollLeft};
  ...
  return <>
    ...
    <section className="panel"><div className="toolbar"><input className="search-input" value={q} onChange={e=>setQ(e.target.value)} placeholder="ค้นหา Location / Item ID / Part Name / Detail / Maker..."/><select value={warehouse} onChange={e=>setWarehouse(e.target.value)}><option value="">คลังทั้งหมด</option><option value="MM-4">Phase4</option><option value="MM-11">Phase11</option></select><button className="btn ghost" onClick={load}>รีเฟรช</button></div>
      {loading?<div className="empty">กำลังโหลด...</div>:<><div ref={topScrollRef} className="table-scroll-top" onScroll={syncTop}><div style={{width:tableScrollWidth}} /></div><div ref={tableScrollRef} className="table-wrap dashboard-stock-wrap" onScroll={syncTable}><table className="dashboard-stock-table"><thead><tr><th>รูป</th><th>Warehouse</th><th>Location</th><th>Item ID</th><th>Part Name</th><th>Part Detail</th><th>Maker</th><th>Stock</th><th>Unit</th><th>Min</th><th>จำนวนที่ Order</th><th>Vendor</th><th>ราคา</th><th>Action</th></tr></thead><tbody>{rows.map(p=><tr key={p.id}><td><div className="table-thumb"><PartImage src={p.image_path} alt={p.name}/></div></td><td>{p.warehouse_label}</td><td>{p.location_code||"-"}</td><td><b>{p.sku}</b></td><td className="part-name-cell">{p.name}</td><td className="detail-cell">{p.description||"-"}</td><td>{p.maker_name||"-"}</td><td className={Number(p.stock_qty)<Number(p.min_stock)?"text-danger":""}><b>{fmt(p.stock_qty)}</b></td><td>{p.unit_code}</td><td>{fmt(p.min_stock)}</td><td>{fmt(p.reorder_qty)}</td><td>{p.supplier_name||"-"}</td><td>{money(p.last_purchase_price)}</td><td><div className="row-actions">{auth.can("can_edit_parts")&&<button className="mini" onClick={()=>setPartModal(p)}>แก้ไข</button>}{auth.can("can_adjust_stock")&&<button className="mini" onClick={()=>setAdjust(p)}>ปรับยอด</button>}{auth.can("can_issue_stock")&&<button className="mini danger" onClick={()=>setStockModal({mode:"issue",part:p})}>เบิก</button>}{auth.can("can_receive_stock")&&<button className="mini success" onClick={()=>setStockModal({mode:"receive",part:p})}>รับเข้า</button>}</div></td></tr>)}</tbody></table></div></>}
    </section>
