...
function StockModal({ mode, part, options, employee, onClose, onSaved }) {
  ...
  function findByLabel(list, label, kind) {
    ...
    return list.find(x => {
      const s = kind === "employee" ? `${x.employee_code} · ${x.name}` : `${x.code} · ${x.name}`;
  ...
  return <Modal title={issue?"เบิกอะไหล่":"รับอะไหล่เข้าสต็อก"} onClose={onClose}>
    <form onSubmit={save}>
      ...
      <div className="form-grid">
        ...
        {issue && <>
          <label className="field"><span>ผู้เบิก *</span><input list="emp-stock" value={requester} onChange={e=>setRequester(e.target.value)} placeholder="พิมพ์ค้นหา..."/><datalist id="emp-stock">{(options.employees||[]).map(x=><option key={x.id} value={`${x.employee_code} · ${x.name}`}/>)}</datalist></label>
        ...
        <label className="field span2"><span>ผู้บันทึก</span><input readOnly value={`${employee?.employee_code||""} · ${employee?.name||""}`} /></label>
